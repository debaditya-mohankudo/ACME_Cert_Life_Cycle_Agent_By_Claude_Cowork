#!/usr/bin/env python3
"""Build TurboVec RAG embeddings for ACME agent Python source and docs.

Chunks each .py file by function/class body (AST-based) and each .md file in
doc/ by ## section. Each chunk maps to a stable uint64 ID derived from
...
Usage:
    ...
    uv run python scripts/build_code_embeddings.py --files agent/nodes/planner.py
"""
...
def _incremental_upsert(rel_paths: list[str]) -> None:
    ...
    # Remove old vectors for files being updated
