...
class OrderInitializerNode:
    """Callable order initializer implementation."""
    ...
    def run(self, state: AgentState) -> dict:
        ...
        logger.info("Order created for %s — %d authorization(s)", domain, len(auth_urls))
        return {
...
def order_initializer(state: AgentState) -> dict:
    """Compatibility wrapper delegating to `OrderInitializerNode`."""
    return OrderInitializerNode().run(state)
